...
## Pas 1.5 — Triaj cross-lună al facturilor din tampon (independent de luna țintă)

**De ce există acest pas.** Utilizatorul trimite facturi în tampon oricând — când le primește, nu neapărat când rulează skill-ul. Fără acest pas, Pas 4.5 (mai jos) verifică tamponul doar contra plăților **lunii curente**, ceea ce lasă două găuri reale: (1) dacă utilizatorul sare o lună, acea lună nu mai devine niciodată „luna curentă", deci o factură pentru ea nu se mai potrivește cu nimic; (2) dacă o factură ajunge după ce pachetul lunii ei a fost deja închis și urcat pe OneDrive, rămâne orfană în tampon la nesfârșit. Acest pas rulează **o singură dată, la începutul fiecărei invocări a skill-ului**, înainte de orice legat de luna țintă curentă, și verifică tamponul contra **tuturor lunilor deja procesate anterior** — nu contra lunii curente (asta rămâne exclusiv treaba Pasului 4.5, neschimbat, mai târziu).

Mecanismul de citire/scriere pe OneDrive pentru acest pas (și pentru Pasul 4.5) e workflow-ul n8n **`OneDrive File Ops (on-demand)`** — vezi `references/n8n-workflow-onedrive-fileops.md` pentru schema completă, exemplele `curl`, și avertismentele găsite prin testare live (numele câmpului binar la upload, modul de răspuns pentru liste). Publică-l doar cât ai nevoie de el, dezactivează-l imediat după, la fel ca la Pasul 2.
...
1. **Enumeră fișierele `Plati *.xlsx` existente**, prin navigare explicită (`getChildren`, repetat pe structura de foldere — nu folosi `file/search`, e o căutare de text pe tot drive-ul, nu una scopată pe folder, și acest skill are deja lecția că operatorii de căutare nesiguri nu trebuie presupuși funcționali fără test live). Limitează la **anul curent + anul anterior** (2 ani) — convenția `Plati.xlsx` există abia din 2026-08, nimic mai vechi nu poate avea așa ceva:
   ...
   ```
2. **Dacă enumerarea nu găsește niciun fișier** (prima rulare vreodată a automatizării, sau nicio lună procesată complet încă): oprește-te aici, raportează pe scurt („niciun `Plati.xlsx` existent încă — sar peste triajul cross-lună") și treci direct la Pasul 2. Nu lista tamponul, nu descărca nimic — nu există niciun caz special de tratat, comportamentul corect rezultă direct din enumerarea goală.
3. Altfel, descarcă fiecare `Plati.xlsx` găsit (prin `OneDrive File Ops`, operația `download`) și construiește o listă unică cu toate rândurile marcate „Factura primită: nu" (sau fără acea coloană deloc, tratate ca deschise) din toate lunile găsite — bazinul de plăți încă neacoperite.
4. Listează tamponul `_Facturi Primite (netriate)` (`getChildren`). Dacă e gol, oprește-te aici.
5. Deschide efectiv fiecare fișier din tampon (la fel ca la Pasul 4.5 — numele fișierului nu conține data reală a facturii) și încearcă o potrivire cu bazinul de la pasul 3, cu aceeași euristică de plauzibilitate ca la Pasul 4.5 (~30 zile, sumă/furnizor plauzibile). **Nu te baza doar pe sumă+furnizor** — la testare live, o factură reală (ADS0031, emisă 30/07/2026, 300 RON, „Servicii de contabilitate luna Iulie 2026") avea aceeași sumă și același furnizor ca o plată deja înregistrată în `Plati 07 - Iulie.xlsx` (20/07/2026, -300 RON), dar rândul acela era de fapt plata pentru o factură complet diferită (`ADS0016/30.06.2026`, menționată explicit în coloana „Referință"). O potrivire naivă ar fi asociat greșit ADS0031 cu o plată deja făcută pentru altă factură. Verifică întotdeauna: (a) plauzibilitatea temporală (factura trebuie să *preceadă* plata, nu invers — aici factura din 30/07 e *după* plata din 20/07, deci exclusă automat de acest criteriu), și (b) dacă rândul de plată are deja un număr de factură menționat în „Referință"/„Observații", acela trebuie să corespundă cu factura pe care încerci s-o potrivești, nu doar suma și furnizorul. Când ambiguitatea nu se rezolvă clar, lasă factura netriată — nu forța o potrivire.
6. Pentru fiecare potrivire: confirmă pe scurt în chat (furnizor + sumă + luna cu care s-a potrivit), mută fișierul din tampon în `Primite/` al lunii respective (`OneDrive File Ops`, operația `move`), redenumește-l curat (operația `rename`), apoi actualizează `Plati.xlsx` al acelei luni (descarcă → editează local cu skill-ul `xlsx`, marchează rândul ca „da" → reîncarcă cu operația `upload`, care suprascrie la același nume).
7. Pentru fișierele rămase nepotrivite: nu le atinge. Raportează un număr neutru („N facturi rămân netriate în tampon, probabil pentru o lună neprocesată încă") — fără listă detaliată, ca să nu pară o problemă la fiecare rulare normală. Separat, semnalează explicit orice fișier a cărui dată reală a facturii (citită la pasul 5) e mai veche de **~60 de zile**, fără nicio potrivire nicăieri — acelea sunt candidați reali la „factură orfană", nu zgomot normal.
...
## Pas 4.5 — Reconciliere cu facturile primite (luna curentă)

Lista de plăți de la Pasul 4 nu e livrabilul final pentru `Primite/` — e punctul de plecare pentru a aduna facturile de la furnizori care corespund plăților identificate. Tamponul verificat aici e ce-a rămas **după** triajul retroactiv de la Pasul 1.5 — orice factură pentru o lună deja procesată anterior a fost deja extrasă acolo, deci ce găsești acum aici e fie pentru luna curentă, fie pentru o lună viitoare, încă neprocesată.
...
1. **Verifică folderul tampon** (după ce Pasul 1.5 a extras deja potrivirile din alte luni): listează conținutul lui prin `OneDrive File Ops` (on-demand), operația `getChildren` — vezi `references/n8n-workflow-onedrive-fileops.md`. Fiecare fișier are numele `AAAA-LL-ZZ_messageId_NumeOriginalAtasament.pdf` — data din nume e **data primirii emailului**, nu data facturii, iar numele original al atașamentului poate fi orice (generic sau informativ) — nu te baza pe numele fișierului pentru identificare.
...
3. Pentru fiecare plată din tabelul de la Pasul 4 care ar trebui să aibă o factură de furnizor (exclude transferuri interne și taxe/buget de stat), caută printre facturile citite din tampon una a cărei dată/sumă/furnizor corespund plauzibil (data facturii precede de regulă plata cu până la ~30 de zile). **Nu te baza doar pe sumă+furnizor** — vezi exemplul concret la Pasul 1.5 (factura ADS0031 vs. plata pentru ADS0016) pentru un caz real în care asta ar fi produs o potrivire greșită. Verifică și dacă „Referință"/„Observații" din rândul de plată menționează deja alt număr de factură.
4. Pentru fiecare potrivire găsită: confirmă-o pe scurt în chat (sumă + furnizor), apoi **mută** fișierul (nu copiază) din folderul tampon în `NN - NumeLună/Primite/`, redenumindu-l curat (`Factura_Furnizor_AAAA-LL-ZZ.pdf`) — prin `OneDrive File Ops`, operațiile `move` + `rename`, ca folderul tampon să rămână curat, cu doar facturile încă neconfirmate.
...
6. Actualizează fișierul `Plati NN - NumeLună.xlsx` cu o coloană/observație „Factura primita: da/nu" — descarcă-l, editează-l local cu skill-ul `xlsx`, reîncarcă-l prin `OneDrive File Ops` (operația `upload`, suprascrie la același nume) — sau raportează separat, în chat, orice plată încă neacoperită de o factură.
...
Dacă workflow-ul de upload OneDrive nu există încă, folosește `OneDrive File Ops (on-demand)` (operația `upload`) — vezi `references/n8n-workflow-onedrive-fileops.md` — în loc să construiești unul separat de la zero; a fost deja construit și testat pentru Pas 1.5/4.5, servește la fel de bine și acest pas.
...
- Triajul cross-lună (Pas 1.5): câte facturi s-au potrivit retroactiv cu luni anterioare, câte au rămas netriate, și dacă vreuna netriată e „veche" (peste ~60 zile) și merită semnalată explicit utilizatorului
