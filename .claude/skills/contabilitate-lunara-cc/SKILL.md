...
Utilizatorul poate trimite facturile de la furnizori oricând în cursul lunii, nu doar când rulează acest skill — le redirecționează (forward) sau le trimite direct pe email către alias-ul `gogu.samsung+facturiprimite@gmail.com` (plus-addressing Gmail, livrează în aceeași cutie, fără cont/alias separat de configurat), cu subiectul rescris ca `[FACTURA] AAAA-LL-ZZ Nume scurt furnizor` (data facturii, nu data emailului). Nu e nevoie de niciun label manual — filtrarea se face după adresa de destinatar. Workflow-ul n8n recurent `Facturi Primite - Staging Sync` (activ permanent, vezi `references/n8n-workflow-facturi-staging.md`) le preia automat zilnic și le pune în folderul tampon OneDrive `Online Leads/Documente Societate/Facturi/_Facturi Primite (netriate)/`.
